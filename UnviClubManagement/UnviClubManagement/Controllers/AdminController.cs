...
namespace UnviClubManagement.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        {
            var clubs = await _db.ClubUpdates
                .Select(c => new {
                    c.Id,
                    c.OldName,
                    c.NewName,
        ...
        [HttpPut("AcceptedUpdatedClubsRequest")]
        public async Task<IActionResult> AcceptUpdatedClub(int clubId)
        {
            var club = await _db.ClubUpdates.FirstOrDefaultAsync(c => c.Id == clubId);

            var clubToUpdate = await _db.Clubs.FirstOrDefaultAsync(c => c.Id == clubId);

            if (club == null)
            {
                return BadRequest("id does not exist or its not Updated");
            }

            clubToUpdate.ClubName = club.NewName;
            clubToUpdate.Description = club.NewDescription;

            _db.ClubUpdates.Remove(club);

            await _db.SaveChangesAsync();
            return Ok();
        }

        [HttpDelete("RejectedUpdatedClubsRequest")]
        ...
        {
            var Event = await _db.EventUpdates
                .Select(e => new {
                    e.Id,
                    e.OldName,
                    e.NewName,
        ...
        [HttpPut("AcceptedUpdatedEventRequest")]
        public async Task<IActionResult> AcceptUpdatedEvent(int eventId)
        {
            var _event = await _db.EventUpdates.FirstOrDefaultAsync(e => e.Id == eventId);

            var eventToUpdate = await _db.Events.FirstOrDefaultAsync(e => e.Id == eventId);
            ...
            _db.EventUpdates.Remove(_event);
